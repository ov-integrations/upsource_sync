...
class Issue:
    def __init__(self, url_onevizion, login_onevizion, pass_onevizion, product_onevizion, trackor_type):
        self.url_onevizion = UrlSetting().url_setting(url_onevizion)
    ...
    def get_issue_list_for_review(self):
    ...
    def add_url_to_issue(self, issue_id, url_upsource, project_upsource, review_id):
        self.issue_list_request.update(
            trackorId=issue_id,
            fields={'I_CODE_REVIEW':url_upsource + project_upsource + '/review/' + review_id}
            )

    def get_issue_list_by_title(self, issue_title):
        self.issue_list_request.read(
